...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_F && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		...
		if (theWindow->articulacion1 > 40.0f)
		{
			theWindow->articulacion1 = 40.0f;
		}
	}
	...
	if (key == GLFW_KEY_H && action == GLFW_PRESS) 
	{
		...
		theWindow->LuzDel = true;
		theWindow->LuzTras = false;
	}
	...
	if (key == GLFW_KEY_J && action == GLFW_PRESS) {
		//Prende luz trasera
		theWindow->LuzTras = true;
		theWindow->LuzDel = false;
	}

	//LLANTAS IZQUIERDA/DERECHA---------------------------------------------
	if (key == GLFW_KEY_K && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		theWindow->articulacion3 += 10.0f;
		if (theWindow->articulacion3 > 20.0f)
		{
			theWindow->articulacion3 = 20.0f;
		}
	}
